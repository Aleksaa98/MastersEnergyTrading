...
exports.toggleUserBlock = async (req, res) => {
    ...
    try {
        ...
        let newType, newBatteryState;

        if (user.type === 'blocked') {
            // Unblock
            newType = 'customer';   // or restore old type if you keep track
            newBatteryState = 'idle';
        } else {
            // Block
            newType = 'blocked';
            newBatteryState = 'blocked';
        }
        ...
        // Update all their batteries
        await Battery.updateMany(
            { traderId: user._id.toString() },
            { $set: { state: newBatteryState } }
        );

        res.status(200).json({ message: `User ${newType}`, user });
    } catch (err) {
